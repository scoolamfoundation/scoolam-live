...
export default function ProfileViewSheet({ visible, onClose }: ProfileViewSheetProps) {
  ...
  // ── same pendingNav pattern as SettingsSheet to avoid real-device routing issues ──
  ...
  useEffect(() => {
    if (visible) {
      // Immediately show cached session data — no spinner needed
      setProfile({
        name: user?.name ?? '',
        email: user?.email ?? '',
        phone: profile.phone,
        ...
        favourite_subjects: profile.favourite_subjects,
  ...
  // Navigate AFTER the modal has been fully dismissed
  useEffect(() => {
          ...
          >
            {profile.phone ? (
              ...
              >
                ...
                <View>
                  <Text style={{ fontSize: 11, color: '#9CA3AF', fontWeight: '600' }}>PHONE</Text>
            ...
            {profile.favourite_subjects?.length ? (
              ...
              >
                ...
                {profile.favourite_subjects.map((s) => (
                  ...
                  >
                    <Text style={{ fontSize: 12, color: '#0D4C3E', fontWeight: '700' }}>{s}</Text>
                  </View>
                ))}
              </View>
            ) : null}
