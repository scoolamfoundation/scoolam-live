...
async function logIAPPurchase(success: boolean, pkg: any) {
  try {
    const planName = pkg?.product?.title ?? pkg?.product?.identifier ?? 'IAP Package';
    const price = pkg?.product?.price ?? 0;
    const platform = Platform.OS;
    await fetch('/api/iap-notification', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        event_type: success ? 'purchase_success' : 'purchase_failed',
        plan_name: planName,
        amount: price,
        platform,
      }),
...
export async function executePurchase({
  ...
  void logIAPPurchase(false, pkg).catch(() => {});
  try {
    const { customerInfo } = await Purchases.purchasePackage(pkg);
    await fetchSubscriptionStatus(setIsSubscribed);
    // Log successful purchase
    void logIAPPurchase(true, pkg).catch(() => {});
