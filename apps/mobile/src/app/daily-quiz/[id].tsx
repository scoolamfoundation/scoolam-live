...
export default function DailyQuizScreen() {
  ...
  // Track which questions were answered incorrectly for retake logic
    ...
    (async () => {
      try {
        await fetch('/api/quiz-attempt', {
          ...
          body: JSON.stringify({ challenge_id: Number(id), score, total: questions.length }),
        });
      } catch {
        /* ignore */
      }
  ...
  const loadQuiz = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/daily-challenge/${id}`);
      if (!res.ok) throw new Error('Failed to load');
      const data = await res.json();

      const c: ChallengeMeta = {
        id: data.challenge.id,
        title: data.challenge.title,
        quiz_duration: data.challenge.quiz_duration ?? 30,
        total_questions: data.challenge.total_questions ?? 5,
        shuffle_questions: data.challenge.shuffle_questions ?? true,
      };
      setMeta(c);

      let qs: Question[] = (data.questions ?? []).filter((q: Question) => q.enabled);
      if (c.shuffle_questions) qs = shuffle(qs);
      qs = qs.slice(0, c.total_questions);
  ...
  // handleRetake is defined inside the finished phase block below
  ...
  if (phase === 'loading') {
    return (
      ...
      >
        <Text style={{ color: '#6B7280', fontSize: 16 }}>Loading challenge…</Text>
      </View>
    );
  }
  ...
  if (phase === 'finished') {
    ...
    return (
      <ChallengeComplete
        ...
        title={meta?.title ?? 'Daily Challenge'}
  ...
  const getOptionStyle = (oi: number) => {
    if (!isReviewing) return { bg: '#fff', border: '#E5E7EB', text: '#374151' };
    ...
    <View style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
        ...
        {questions.map((_, i) => (
          <View
            ...
            style={{
              ...
              borderRadius: 4,
              backgroundColor:
                i < currentIdx ? '#F59E0B' : i === currentIdx ? '#F59E0B' : '#E5E7EB',
            }}
          />
        ))}
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 140 }}>
        ...
        <Text style={{ color: '#6B7280', fontWeight: '600', fontSize: 13, letterSpacing: 0.5 }}>
          DAILY CHALLENGE – Q{currentIdx + 1}
        </Text>
        <Text
          style={{
            fontSize: 21,
