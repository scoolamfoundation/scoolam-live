...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await sql`SELECT * FROM infographics WHERE id = ${id}`;
  if (!rows[0]) return Response.json({ error: 'Not found' }, { status: 404 });
  return Response.json({ infographic: rows[0] });
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { title, description, thumbnail_url, file_url, is_premium } = body;
  ...
  const result = await sql(
    `UPDATE infographics SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`,
    values
  );
  if (!result[0]) return Response.json({ error: 'Not found' }, { status: 404 });
  return Response.json({ infographic: result[0] });
}
